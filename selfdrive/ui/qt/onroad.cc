...
void OnroadHud::paintEvent(QPaintEvent *event) {
  ...
  if (geoRecord == 2)
    drawIcon(p, rect().right() - radius / 2 - bdr_s * 2, rect().bottom() - footer_h / 2 + 50, geo_rec_on, QColor(0, 0, 0, 70), 1.0);
  else if (geoRecord == 1)
    drawIcon(p, rect().right() - radius / 2 - bdr_s * 2, rect().bottom() - footer_h / 2 + 50, geo_rec_off, QColor(0, 0, 0, 70), 1.0);
  else
    drawIcon(p, rect().right() - radius / 2 - bdr_s * 2, rect().bottom() - footer_h / 2 + 50, geo_rec_nogps, QColor(0, 0, 0, 70), 1.0);
